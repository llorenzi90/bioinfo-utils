...
then
	...
	trim_galore --length 35 --fastqc $read1 
	echo -e "################################\n\tTrimGalore done\n################################\n"
	echo `date  +'%r'`
	if $only
	then 
...
if [ $runstep == all ] || [ $runstep == $step ]
then
	read1_trimmed=$(ls *trimmed.fq.gz)
	if ! [ -f "$read1_trimmed" ]; then
		die "input file $read1_trimmed not found"
	fi
	
	echo "read1: $read1_trimmed"
	...
	bowtie2 --verbose --very-sensitive -x $bowtie2Index -U $read1_trimmed -S $sample.bt2.sam
	echo -e "################################\n\tBowtie2 done\n################################\n"
	echo `date  +'%r'`
...
#6) MAPQ distribution, filtering and stats
step=filtstats
if [ $runstep == all ] || [ $runstep == $step ]
then 
	...
	then
		...
		#A) FILTERS: primary chromosomes only (exclude chrM reads), and MAPQ > 2
		...
		#A.2) retain only reads with minimum MAPQ=2
		samtools view -@ $PPN -b -h -q2 $sample.sorted.markedDups.primary_chr.bam > $sample.sorted.markedDups.primary_chr.minq2.bam

		#B) MAPQ distribution for raw reads with and without chrM
		...
		for fi in $sample.sorted.markedDups $sample.sorted.markedDups.primary_chr $sample.sorted.markedDups.primary_chr.minq2
		do
		...
		samtools index -@ $PPN $sample.sorted.markedDups.primary_chr.minq2.bam
		...
		#A.1) retain only reads with minimum MAPQ>=2
		samtools view -@ $PPN -b -h -q2 $sample.sorted.markedDups.bam > $sample.sorted.markedDups.minq2.bam

		#B) MAPQ distribution for raw reads 
		raw=$sample.sorted.markedDups
		samtools view $raw.bam |cut -f5 |sort -n |uniq -c > $raw.MAPQdist

		#C) STATS and fragment lenght distributions for both bam files (raw and qulity filter) 
		for fi in $sample.sorted.markedDups $sample.sorted.markedDups.minq2
		do
			samtools stats $fi.bam > $fi.stats
			samtools view $fi.bam | awk '$9>0' | cut -f 9 | sort | uniq -c | sort -b -k2,2n | sed -e 's/^[ \t]*//' > $fi.fragment_length_count.txt
		done
		...
		#D) index the final file
		samtools index -@ $PPN $sample.sorted.markedDups.minq2.bam
...
then
	...
	then
		...
		input=$sample.sorted.markedDups.minq2.bam
	fi
